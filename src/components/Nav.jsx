...
export default function Nav() {
  ...
  return (
    <Disclosure as="nav" className="bg-gray-800 h-[8vh]">
      <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div className="relative flex h-16 items-center justify-between">
          <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
            <DisclosureButton
              className="group relative inline-flex items-center justify-center roundhttp://localhost:5173/abouted-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
              onClick={() => setIsMenuOpen(!isMenuOpen)}
            >
              <span className="sr-only">Open main menu</span>
              <Bars3Icon aria-hidden="true" className={`h-6 w-6 ${isMenuOpen ? 'hidden' : 'block'}`} />
              <XMarkIcon aria-hidden="true" className={`h-6 w-6 ${isMenuOpen ? 'block' : 'hidden'}`} />
            </DisclosureButton>
          </div>
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            <Menu as="div" className="relative ml-3">
              ...
              <Menu.Items
                className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-gray-50 py-1 shadow-lg ring-1 ring-black ring-opacity-5 transition focus:outline-none"
              >
                <div className="px-4 py-2">
                  {user && (
                    <span className="block text-sm text-gray-700">
                      Email : {user.email} <br />Role : ({roles.join(', ')})
                    </span>
                  )}
                </div>
                {!user && (
        ...
        <div className="space-y-1 px-2 pb-3 pt-2">
          {navigation.map((item) => (
            <a
              ...
              aria-current={item.current ? 'page' : undefined}
              className={classNames(
                item.current ? 'bg-gray-900 text-white' : 'text-purple-300 bg-gray-700 hover:bg-gray-950 hover:text-white w-[40vw]',
